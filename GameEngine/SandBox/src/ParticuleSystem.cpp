...
void ParticleSystem::OnRender(GameEngine::OrthographicCamera& camera)
{
	GameEngine::Renderer2D::BeginScene(camera);
	for (auto& particle : m_ParticlePool)
	{
		...
		GameEngine::Renderer2D::DrawRotatedQuad(position, { size, size }, particle.Rotation, color);
